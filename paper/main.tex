...
demonstrated this on Overcooked, a two-player common-payoff coordination
game (which we re-skin as a diamond-cutting atelier in
Section~\ref{sec:prelim}, matching our deployment domain):
...
\paragraph{Overcooked as a diamond atelier.}
Our testbed is the Overcooked benchmark~\citep{carroll2019utility}, which
we present throughout this paper---and render in the released demo---under
a \emph{diamond-atelier skin} matched to Jewel Labs' domain: two workers in
a gridworld workshop must produce and deliver finished diamonds. A worker
fetches \emph{rough stones} (onions, in the benchmark's original kitchen
skin) and loads three of them into a \emph{cutting wheel} (pot); after 20
timesteps of processing, the finished \emph{diamond} (soup) is collected
onto a \emph{presentation tray} (dish) and handed over at the
\emph{client counter} (serving window) for a shared reward of 20 per
delivery. Only the vocabulary and rendering are ours; the dynamics,
layouts, and human data are the unmodified benchmark, so every number
remains comparable to the Overcooked literature. Episodes last $H=400$
timesteps. Movement is simultaneous; workers collide, block corridors, and
must pass stones over counters on some layouts. We use all five classic
layouts (Figure~\ref{fig:layouts-curves}):
...
\emph{Forced Coordination} (neither worker can complete a diamond alone---
stones and trays must be passed across a divider), and
\emph{Counter Circuit} (long circuit; passing stones over the central
counter is the expert strategy). Following the benchmark's human-data
configuration, we use the 2019 (``old'') dynamics in which the wheel
starts automatically when the third stone is loaded. Observations are the
...
\paragraph{Reward.}
All RL methods optimize the shared sparse reward (+20 per delivered
diamond) plus the benchmark's per-player shaped rewards (stone loaded into
the wheel $+3$, tray pickup $+3$, finished-diamond pickup $+5$) scaled by
a coefficient annealed linearly
...
\begin{table}[t]
\centering
\caption{Mean episode score (delivered diamonds $\times$ 20, horizon 400)
when paired
...
\caption{\textbf{Top:} the five layouts (orange = rough-stone dispensers,
blue = tray dispensers, dark = cutting wheels, green = client counters).
\textbf{Bottom:}
...
Coordination---where neither worker can complete a diamond alone---SP with a
...
layouts, no diamond can be delivered at all, making those the
