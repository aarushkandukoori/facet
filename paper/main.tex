...
\author{%
  Aditya Kandukoori\thanks{Jewel Labs Research (\url{https://jewellabs.org}). Correspondence: \texttt{research@jewellabs.io}.} \\
...
trained with a fixed human model (\ppobc{}), and \facet{}, each with three
seeds per layout, and evaluate every agent along two axes of the workspace
problem: coordination with \emph{held-out human proxies} and with an
\emph{existing fleet} of independently trained agents. The classic failure
reproduces sharply at laptop scale: self-play loses 53--96\% of its
self-paired score when moved to human proxies. No single method dominates
both axes---\ppobc{} is the strongest human-proxy method (best on 3/5
layouts) but sacrifices fleet compatibility, while self-play shows the
reverse profile. \facet{} is the only method that stays functional with
every partner type on every layout: its worst evaluation cell scores 12.7
versus 3.7--5.8 for all baselines, and it attains the best combined
human+fleet score (110.3 vs.\ 102.0 for \ppobc{} and 95.0 for SP). All
code, all 75 trained models, and an interactive browser demo are
released.\footnote{\url{\repourl{}}}
\end{abstract}
...
themselves yet dropped below a simple imitation baseline when paired with
human proxies. \citet{hu2020otherplay} formalized the underlying issue as
...
data by training a best response to a diverse population, most prominently
Fictitious Co-Play (FCP)~\citep{strouse2021collaborating}, which uses
independent self-play runs \emph{and their past checkpoints} as partners;
successors refine the population's diversity via entropy
bonuses~\citep{zhao2023maximum}, trajectory
divergence~\citep{lupu2021trajectory}, hidden-utility
biases~\citep{yu2023hsp}, or generative partner
...
human-compatibility method, but it pays for that specialization on the
machine-partner axis, where \facet{} beats it on four of five layouts. No
method wins everywhere; what the multiplayer-workspace setting arguably
demands is the \emph{absence of catastrophic cells}, and there \facet{} is
alone: its worst score across all ten (layout $\times$ partner-type) cells
is 12.7, versus 3.7 for SP, 5.3 for \ppobc{}, and 5.8 for BC, and its
combined human+fleet average (110.3) is the highest of the four methods.
...
pairing. \facet{} borrows FCP's checkpoint zoo, MEP's prioritization, and
...
\paragraph{\facet{}.}
PPO best response to a \emph{prioritized population}
...
($\sim$50k parameters); BC models are $96\!\to\!64\!\to\!64\!\to\!6$.
Training budget is 5M environment steps per run on Cramped Room, Asymmetric
Advantages, and Coordination Ring, and 7M on the harder Forced Coordination
and Counter Circuit. Every number in this paper was produced on one Apple M4
MacBook (10 cores, no GPU); a single PPO run sustains $\sim$4.5--5k
environment steps/second, and the full 45-run RL suite plus evaluation
completes in under six hours. Complete hyperparameters in
...
these SP runs, so for \facet{} this axis measures \emph{retained}
...
are unseen. Cross-seed SP$+$SP pairing is the classic zero-shot
...
human--human test games for reference. Bold = best-with-\hproxy{} per
layout (ties within one s.e.\ also bold).}
\label{tab:main}
\small
...
is unseen. Bold = best per layout (ties within one s.e.).}
\label{tab:fleet}
\small
\input{tables/fleet_table}
\end{table}
...
15-partner population: \facet{} trails \ppobc{} on the human axis on three
layouts, though it clearly beats SP where SP collapses hardest (84.4 vs.\
60.0 on Asymmetric Advantages; 35.3 vs.\ 5.8 on Forced Coordination) and
matches \ppobc{} on Forced Coordination within two s.e. Counter Circuit is
...
ever completing a delivery chain. Cross-seed SP$+$SP pairing is strong where independent
...
\paragraph{No catastrophic cells.}
...
95.0 for SP, and 43.3 for BC. More telling than the average is the
minimum: across all ten (layout $\times$ partner-type) cells, \facet{}'s
worst score is 12.7, whereas SP bottoms out at 3.7, \ppobc{} at 5.3, and
BC at 5.8 (Figure~\ref{fig:gap} visualizes the self-vs-proxy gaps). In a
deployed workspace, the worst cell is the one users remember.
...
concentrates on the three BC human models ($\approx$55\% of sampling, vs.\
...
evaluation is the field's standard first filter but is known to compress
differences between methods~\citep{strouse2021collaborating,wang2024zsceval},
and a user study remains future work. Second, all agents use the benchmark's
...
production setting. Finally, \facet{} inherits FCP's main cost: it consumes
the checkpoints of multiple self-play runs, though these are artifacts most
...
catastrophic cells when agents must work with humans \emph{and} an
existing fleet---the defining condition of a multiplayer workspace. For
...
\input{tables/perseed_table}

\section{Reproducibility}
\label{app:repro}

All experiments are reproducible from the released repository:
\texttt{scripts/train\_bc.py} builds the datasets and BC models
($\sim$25 minutes), \texttt{scripts/run\_batch.sh} executes the three
PPO stages (SP, then \ppobc{} and \facet{}; $\sim$5 hours on a 10-core
M4), and \texttt{scripts/evaluate.py} regenerates every table and figure
from raw episode scores. Random seeds are fixed (0, 1, 2) at every stage.
